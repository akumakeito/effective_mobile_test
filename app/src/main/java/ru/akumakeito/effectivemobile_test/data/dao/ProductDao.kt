...
interface ProductDao {
    ...
    fun getAllSortedByPopular(): List<ProductEntity>

    @Query(
        "SELECT * FROM products ORDER BY " +
                "CASE WHEN :isAsc = 0 THEN priceWithDiscount END DESC, " +
                "CASE WHEN :isAsc = 1 THEN priceWithDiscount END ASC"
    )
    fun getAllSortedByPrice(isAsc: Boolean): List<ProductEntity>
